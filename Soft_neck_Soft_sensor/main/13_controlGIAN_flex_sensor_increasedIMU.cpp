...
int main()
{
    ...
    //--Frequencies---------------------------------------------------
    double freqIMU=50; //IMU sensor use values: 50,100,500... (better keep on 50 to avoid issues)
    double freqControl=2; //Multimeter does around 2 measurements per seconds
    double dtsIMU=1/freqIMU;
    double dtsControl=1/freqControl;
    double dts = gcd(dtsIMU*1000,dtsControl*1000)/1000;
    
    SamplingTime Ts;
    SamplingTime TsControl;

    Ts.SetSamplingTime(dts);
    TsControl.SetSamplingTime(dtsControl);

    //--IMU Sensor---------------------------------------------------
    cout << "------------------------------------" <<  endl;
    
    IMU3DMGX510 misensor("/dev/ttyUSB0",freqIMU);  // IMU port selection (portName,frequency)
...
//    FPDBlock Cp(18.5588,1.5752,0.071226);
    FPDBlock Cp(2.8824,1.314,0.56809);
    FPDBlock Cr(0,0,1);

    //--IMU Calibration-----------------------------------------------
    for (double t=0;t<5;t+=dtsIMU)
        {
    ...
    //--Plots----------------------------------------------------------
    IPlot roll(dtsIMU, "roll", "xLabel", "yLabel");
    IPlot pitch(dtsIMU, "pitch", "xLabel", "yLabel");
    IPlot resistance_plot(dtsControl, "res", "xLabel", "yLabel");
...
//    referenceRamp(target, dtsControl, 0.5, 0.2); //RAMP->(std::vector<double>& pitch, double dt, double amplitude, double rate): target=vector in which it saves the trajectory, dt=time step
//    target_pitch.insert(target_pitch.end(), target.begin(), target.end());

//    std::vector<double> target1;
//    referenceStep(target1, dtsControl, 0.0, 5.0); //STEP->(std::vector<double>& pitch, double dt, double amplitude, double duration)
//    target_pitch.insert(target_pitch.end(), target1.begin(), target1.end());

//    std::vector<double> target2;
//    referenceDoubleRamp(target2, dtsControl, 0.5, 0.2); //DOUBLE RAMP->(std::vector<double>& pitch, double dt, double amplitude, double rate)
//    target_pitch.insert(target_pitch.end(), target2.begin(), target2.end());

//    std::vector<double> target3;
//    referenceCyclicRamp(target3, dtsControl, 0.5, 0.5, 3); //CYCLIC RAMPS->(std::vector<double>& pitch, double dt, double amplitude, double rate, int nCycles)
//    target_pitch.insert(target_pitch.end(), target3.begin(), target3.end());

//    std::vector<double> target1;
//    referenceChirp(target1, dtsControl, 0.174533, 0.174533, 0.01, 0.8, 100); //CHIRP ->(std::vector<double>& angle, double dt, double amplitude, double offset, double freq0, double freqF, double duration)
//    target_pitch.insert(target_pitch.end(), target1.begin(), target1.end());
    ...
    for (index=0;index<size_preload; index++)
    {
        ...
        timestamps_IMU.push_back(elapsed_time);
        ...
        if (elapsed_time < dtsIMU) std::this_thread::sleep_for(std::chrono::duration<double> (dtsIMU - elapsed_time));
        index_preload = index;
    }
    ...
    while(testing){
        ...
        if (elapsed_time - lastIMUTime >= dtsIMU) {
            misensor.GetPitchRollYaw(rpy[1],rpy[0],rpy[2]);  //reading from IMU
            dR.push_back((data_resistance.back()-r0)/r0); //reading and computing relative resistance
            ...
            lastIMUTime = elapsed_time;
            timestamps_IMU.push_back(lastIMUTime);
        }

        if (elapsed_time - lastControlTime >= dtsControl) {
            ...
            lastControlTime = elapsed_time;
            timestamps_main.push_back(lastControlTime);
            cout << "^^^^^^^^^^^^^^^" << endl;
            index++;
        }
    ...
    exportVectorToCSV(timestamps_IMU, "/home/humasoft/Soft_neck_Soft_sensor/CSVdata/IMU_rpy_time_data.csv");
