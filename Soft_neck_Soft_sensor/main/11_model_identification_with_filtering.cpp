...
double last_res;
void multimeterThreadFunction(SerialComm* serial, std::vector<double>& data_resistance, std::vector<double>& filteredRes, double dt, std::chrono::steady_clock::time_point start_time, bool& running) {
    int index = 0;
    double filteredValue;
    double windowSize = 9; // Example moving average window size
    ...
    while (running) {
        ...
        if (index+1 >= windowSize) {
            // Moving average over the last `windowSize` elements
            double sum = 0;
            for (int i = index+1  - windowSize; i < index+1 ; i++) {
                sum += data_resistance[i];
            }
            filteredValue = sum / windowSize;
        } else {
            // Calculate the average of all elements if the size is smaller than the window
            double sum = 0;
            for (int i = 0; i < index+1; i++) {
                sum += data_resistance[i];
            }
            filteredValue = sum / index+1;
        }
        // else
        //     filteredValue = rel_resistance(i); // Keep original value if not in load cycle
        // end

        // Append filtered value
        filteredRes[index] = filteredValue;
...
//    std::vector<double> target1;
//    referenceChirp(target1, dts, -0.174533, -0.174533, 0.01, 0.4, 100); //CHIRP ->(std::vector<double>& angle, double dt, double amplitude, double offset, double freq0, double freqF, double duration)
//    target_pitch.insert(target_pitch.end(), target1.begin(), target1.end());



    //45 degrees at 20deg/s
    std::vector<double> target2;
    referenceCyclicRamp(target2, dts, 0.523599, 0.174533, 5);
    target_pitch.insert(target_pitch.end(), target2.begin(), target2.end());

//    std::vector<double> target0;
//    referenceStep(target0, dts, 0.0, 5.0); //STEP->(std::vector<double>& pitch, double dt, double amplitude, double duration)
//    target_pitch.insert(target_pitch.end(), target0.begin(), target0.end());
//    std::vector<double> target1;
//    referenceStep(target1, dts, 0.785398, 5.0); //STEP->(std::vector<double>& pitch, double dt, double amplitude, double duration)
//    target_pitch.insert(target_pitch.end(), target1.begin(), target1.end());
//    std::vector<double> target2;
//    referenceStep(target2, dts, 0.0, 5.0); //STEP->(std::vector<double>& pitch, double dt, double amplitude, double duration)
//    target_pitch.insert(target_pitch.end(), target2.begin(), target2.end());
...
//    target_pitch.insert(target_pitch.end(), target3.begin(), target3.end());
    ...
    std::vector<double> filteredRes(size, 0.0);
    ...
    std::thread multimeterThread(multimeterThreadFunction, serial, std::ref(data_resistance), std::ref(filteredRes), dts, start_time, std::ref(running)); //THE COMMAND TO READ THE MULTIMETER TAKES TOO MUCH TIME, SO IT MESSES UP THE CONTROL (this is why it is not inside the control loop)
    ...
    for(double index=0; index<size; index++) {
        cout << "Roll: " << data_rpy[index][0] << " Pitch: " << data_rpy[index][1]  << " Yaw: " << data_rpy[index][2] << " Filtered RES: " << filteredRes[index] << " Time: " << timestamps_main[index] << " s" <<endl;
    }
    ...
    exportVectorToCSV(filteredRes, "/home/humasoft/Soft_neck_Soft_sensor/CSVdata/filteredRes.csv");
